...
function App() {
  ...
  React.useEffect(function() {
    ...
    Promise.all([
      api.listHunts(),
      api.getUserEnrollments(uname)
    ]).then(function(results){
      var allHunts = results[0];
      var enrollments = results[1];
      
      var enrolledIds = enrollments.map(function(e_rec) { return e_rec.hunt_id; });
  ...
  function flash(msg) {
    ...
    setToast(msg);
    flashTimeoutRef.current = setTimeout(function(){
      setToast("");
  ...
  function checkinToPin(pin) {
    requireAuth(function(){
      var isHuntStep = false;
      var activeEnrollGlobal = null;
      var activeStepGlobal = null;
      var huntGlobal = null;

      var proceedCheckin = function() {
            ...
            var handleHuntStepCheckin = function() {
              if (activeEnrollGlobal && activeStepGlobal) {
                ...
                api.getHuntActivityLogs(activeEnrollGlobal.id).then(function(logs) {
                  ...
                  console.error("Hunt checkin failed:", err);
      ...
      if (uname && uname !== 'guest') {
        api.getUserEnrollments(uname).then(function(enrollments) {
          ...
          var checkPromises = activeEnrollments.map(function(enroll) {
            return api.getHunt(enroll.hunt_id).then(function(hunt) {
              return api.getHuntActivityLogs(enroll.id).then(function(logs) {
                ...
                return {
                  ...
                  isMatch: activeStep && activeStep.pin_id === pin.id
                };
              });
            });
          });
          ...
          Promise.all(checkPromises).then(function(results) {
            var match = results.find(function(r) { return r.isMatch; });
            if (match) {
              activeEnrollGlobal = match.enroll;
              activeStepGlobal = match.activeStep;
              huntGlobal = match.hunt;
              isHuntStep = true;
            }
          ...
          e("div",{style:{background:"rgba(42,93,60,0.06)",border:"1px solid #c7dacb",borderRadius:10,padding:12,marginBottom:12}},
            ...
            e("ul",{style:{paddingLeft:16,margin:0,display:"flex",flexDirection:"column",gap:6,fontSize:12}},
              ...
              e("li",null,
                ...
                lang === 'es' ? "Crea una pista para ayudar a los exploradores a descifrar adónde ir." : "Write a riddle, clue, or instructions to collection players to the destination."
              ),
              e("li",null,
          ...
          e("div",{style:{background:"rgba(42,93,60,0.06)",border:"1px solid #c7dacb",borderRadius:10,padding:12}},
            ...
            e("ul",{style:{paddingLeft:16,margin:0,display:"flex",flexDirection:"column",gap:6,fontSize:12}},
              e("li",null,
                ...
                lang === 'es' ? "Colección a amigos por monumentos de tu ciudad usando pistas de historia local." : "Lead friends through town landmarks with fun trivia as clues."
              ),
              e("li",null,

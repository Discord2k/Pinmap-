...
import { getWhatsNewList } from '../utils/helpers';

export function WhatsNew(props) {
  var lang = props.lang || 'en';
  var t = props.t || function(key) {
    if (key === 'whats_new_title') return "What's New";
    if (key === 'whats_new_subtitle') return "Latest updates to PINMAP";
    if (key === 'got_it') return "Got it!";
    return key;
  };
  var updates = getWhatsNewList(lang);
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:9000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#f6f1e4",border:"none",animation:"slideUp 0.32s cubic-bezier(0.34,1.1,0.64,1) both",borderRadius:16,padding:"24px 22px",maxWidth:420,width:"100%",boxShadow:"0 8px 40px rgba(0,0,0,0.28)",maxHeight:"80vh",display:"flex",flexDirection:"column"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
          <div>
            <div style={{fontSize:18,fontWeight:700,color:"#2a5d3c"}}>{t('whats_new_title')}</div>
            <div style={{fontSize:12,color:"#6f786f"}}>{t('whats_new_subtitle')}</div>
          </div>
        ...
        <div style={{overflowY:"auto",flex:1,marginBottom:16}}>
          {updates.map(function(item,i){
            return (
              <div key={i} style={{display:"flex",gap:12,padding:"10px 0",borderBottom:i<updates.length-1?"1px solid #e8dcc4":"none"}}>
